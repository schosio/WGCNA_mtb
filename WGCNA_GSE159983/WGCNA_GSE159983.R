...
gset <- getGEO("GSE159983", GSEMatrix =TRUE, getGPL=FALSE)
if (length(gset) > 1) idx <- grep("GPL16972", attr(gset, "names")) else idx <- 1
gset <- gset[[idx]]

# make proper column names to match toptable 
fvarLabels(gset) <- make.names(fvarLabels(gset))
# this will changege the coloumn
#  name to match the toptable format
...
setwd("/Users/mokira/MPLAB/R_work/Upstream_sequences/Cytoscape3")
